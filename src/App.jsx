...
const App = () => {
    ...
    function addToCart(prodotto) {
        if (!prodottoAggiunto.some((element) => element.name === prodotto.name)) {
    ...
    function updateProductQuantity(prodotto) {
        if (prodottoAggiunto.some((element) => element.name === prodotto.name)) {
            ...
            setProdottiAggiunto(prodottoAggiunto.map((element) => element.name === prodotto.name ? { ...element, quantity: element.quantity + 1 } : element))
        }
    }
                ...
                <div className={style.contenuto}>
                    {prodottoAggiunto.map((element, index) =>
                        <div key={index} className={style.product}>
                            <h4 className={style.noMargin}>Name: {element.name}</h4>
                            <div className={`${style.d_flex} ${style.center}`}>
                                <div className={style.d_flex}>
                                    <p>Quantity: <span className={style.quantity}>{element.quantity}</span></p>
                                    <p>Price: <span className={style.price}>{element.price}</span></p>
                                </div>
                                <button className={style.buttonDelete} onClick={removeFromCart}>Rimuovi Prodotto</button>
                            </div>
                        </div>
                    )}
                </div>
